 ...
 */
function decodeAddress(address: string): CSL.Address {
...
export async function buildTransaction(
  ...
  const { wallet, outputs, metadata, changeAddress, policyId, policyScript, policyKey, mintAssets } = params
  ...
  try {
    ...
    for (const utxoString of utxos) {
      try {
        ...
        if (utxo) {
          ...
          if (totalInputLovelace >= requiredWithFee) {
            break
          }
        }
      } catch (error: any) {
    ...
    if (totalInputLovelace < requiredWithFee) {
      throw new Error('残高が不足しています（手数料を含む）')
    }
    ...
    // 6. メタチE�Eタを追加�E�オプション�E�E
    let txMetadata: CSL.GeneralTransactionMetadata | undefined
    let auxiliaryData: CSL.AuxiliaryData | undefined
    if (metadata) {
      txMetadata = createMetadataCBOR(metadata)
    ...
    // 13. TTLを設宁E
    const slot = CSL.BigNum.from_str(Math.floor(Date.now() / 1000 + 3600).toString()) // 1時間征E
    ...
    // 15. 手数料を設宁E
    const feeAmount = feeEstimate
    txBuilder.set_fee(CSL.BigNum.from_str(feeAmount.toString()))
    ...
    if (policyKey && policyScript && mintBuilder) {
      try {
        ...
        // MintBuilderからネイチE��ブスクリプトを取征E
        const nativeScripts = mintBuilder.get_native_scripts()
        if (nativeScripts && nativeScripts.len() > 0) {
          let witnessNativeScripts = witnessSet.native_scripts()
          if (!witnessNativeScripts) {
            witnessNativeScripts = CSL.NativeScripts.new()
            witnessSet.set_native_scripts(witnessNativeScripts)
          }
          for (let i = 0; i < nativeScripts.len(); i++) {
            witnessNativeScripts.add(nativeScripts.get(i))
          }
        }
...
function mergeCollection(
  ...
  for (let i = 0; i < sourceCollection.len(); i++) {
    targetCollection.add(sourceCollection.get(i))
  }
}
